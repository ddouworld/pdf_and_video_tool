...
class My_Thread(QThread):
    ...
    def __init__(self, mainwindow,ui,name,parent=None):
        ...
        self.ui = ui
        self.fun_name =name
    def set_delete_video_time_path(self,video_path, ad_path, delete_time,num):
    ...
    def run(self):
        if self.fun_name == "delete_video_time":
            ...
            delete_video_time(self.video_path,self.ad_path,self.delete_time)
            self.finishSignal.emit("第{}个视频处理完成".format(self.num))
        elif self.fun_name == "delete_video_time_2":
            self.finishSignal.emit("正在处理第{}个视频".format(self.num))
            delete_video_time_2(self.video_path, self.ad_path, self.delete_time)
            self.finishSignal.emit("第{}个视频处理完成".format(self.num))
        elif self.fun_name == "fixed_watermarking":
            self.finishSignal.emit("固定水印:正在处理第{}个视频".format(self.num))
            fixed_watermarking(self.video_path,self.water_path,self.scale_size,self.show_every_seconds, self.show_duration_seconds,self.x,self.y)
            ...
            delete_pdf_page(self.pdf_path,self.a4path,self.num,True)
            ...
            add_watermark_2(self.pdf_path,self.a4path,self.ppt_path)
